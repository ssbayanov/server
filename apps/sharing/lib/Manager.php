...
class Manager {
	...
	private function validate(Share $share): void {
		if (!UUIDUtil::validateUUID($share->id)) {
			throw new ShareInvalidException('The ID is not a valid UUID.');
		}
	...
	private function validateShareOperation(?IUser $user, Share $share): void {
		if ($user instanceof \OCP\IUser && $share->creator !== $user->getUID()) {
	...
	private function internalList(?IUser $currentUser, ?string $shareID, ?string $sourceType, bool $applySelectors = true, bool $applyFilters = true): array {
		...
		if ($applySelectors) {
			if (!$currentUser instanceof \OCP\IUser) {
		...
		$shares = array_map(static fn (array $share): \OCA\Sharing\Model\Share => Share::fromArray($share), $shares);
